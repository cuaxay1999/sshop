...
import { ACCOUNT_STATE, EMAIL_PATTERN, PHONE_PATTERN } from "../../../../../utils/constants/config";
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (code === "SSHOP-613") {
        ...
        if (resetPwd) {
          forgotPwd(registerInfo.isFirebase, username, true, registerInfo.otpReferenceId)
        }
        ...
        const resMsg = get(data, "status.message", texts?.ERROR_CORE_GENERAL_ERROR);
        message.error(resMsg);
      }
      setCallingAPI(false);
    } catch (error) {
      message.error(texts[error?.data?.status?.label] || error?.data?.status?.message || texts?.ERROR_CORE_GENERAL_ERROR);
      setCallingAPI(false);
    }
  };
  ...
  return (
    <div className="check-phone-number">
      ...
      <div className="form-heading">
        <div className='header-text' >{texts.LABEL_LOGIN} {texts?.LABEL_OR} {texts.LABEL_REGISTER?.toLowerCase()}</div>

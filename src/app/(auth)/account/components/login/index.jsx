    ...
    const handleGetChainList = async () => {
        ...
        if (code == 200) {
            ...
            if (chains.length === 0) {
                ...
                if (!currentChain || (currentChain && !chains.find((c) => c.chainId == currentChain))) {
                    //find available shop
                ...
                const subDomain = chains.find((c) => c.chainId == currentChain)?.subDomain
                const response = await getShopsByChainId(currentChain);

                if (response?.data?.data?.length > 0) {
                    const url = `https://${subDomain}.${CONFIG_SERVER.DOMAIN}?user-token=${Cookies.get(SSHOP_SPA_TOKEN)}&locale=${locale}`;
        ...
        <div className="login-component">
            ...
            >
                ...
                <div style={{textAlign: "center"}}>
                    <Button onClick={() => forgotPwd(authFirebase, mobileNumber)}
                            type='link'
                    > {texts.LABEL_FORGOT_PASSWORD}</Button>
